...
const Header: React.FC<HeaderProps> = ({ lang, setLang, t, hasSelectedLang, theme, setTheme }) => {
  ...
  return (
    <header className="w-full h-16 flex items-center sticky top-0 z-40 transition-all duration-300 shrink-0">
      {/* Liquid Glass Background */}
      <div className={`absolute inset-0 backdrop-blur-xl border-b shadow-sm transition-colors duration-500 ${
        theme === 'dark' 
          ? 'bg-[#0B0F19]/80 border-white/[0.08]' 
          : 'bg-white/80 border-slate-200/80'
      }`}></div>
      
      <div className="relative container mx-auto px-4 lg:px-6 flex items-center justify-between">
        {/* Left: Logo */}
        <div 
          onClick={handleLogoClick}
          className="flex items-center gap-3 group cursor-pointer select-none transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-105"
        >
          ...
          <div className="relative w-9 h-9 flex items-center justify-center">
            ...
            <div className={`absolute inset-0 backdrop-blur-md rounded-xl shadow-inner border flex items-center justify-center group-hover:scale-105 transition-transform duration-300 ${
              theme === 'dark' 
              ...
              <Crop className="w-2.5 h-2.5 text-indigo-100 absolute translate-x-1 translate-y-1" />
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className="relative z-50">
            <button
              ...
              className={`
                flex items-center gap-2 px-3 h-9 rounded-xl border transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-105 group
                ${isLangOpen 
            ...
            {/* Dropdown Menu */}
            <div className={`
              absolute right-0 mt-2 w-40 border rounded-xl shadow-xl overflow-hidden origin-top-right transition-all duration-300 ring-1 backdrop-blur-2xl
               ...
               <div className="py-1">
                  {languages.map((l) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 text-xs flex items-center gap-3 transition-colors border-l-[2px]
                    ...
                    >
                      ...
                      <span>{l.label}</span>
                      {lang === l.code && hasSelectedLang && <Check className="w-3 h-3 ml-auto text-indigo-500" />}
                    </button>
                  ))}
               </div>
            </div>
